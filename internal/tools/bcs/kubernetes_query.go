...
func (t *KubernetesQueryTool) Info(context.Context) (*schema.ToolInfo, error) {
	return &schema.ToolInfo{
		...
		Desc: "通过 BCS 网关只读查询 Kubernetes 原生资源或 CRD，支持 list/get，不执行变更、不读取日志或 Secret。通常只传 kind，程序会在内部通过 API Discovery " +
			...
			"用户仅提供名称关键词（例如 cwlicense）时，使用 list 并传 name_contains，只返回名称包含该片段的资源，不要逐页取全量结果交给模型筛选。完整名称用 get 和 name。name_contains 为大小写敏感的字面匹配，不支持通配符。" +
			...
			"redacted=true 表示凭证等敏感内容已替换，不能称为未经处理的原文。完整资源上限 64 KiB，超限报错，不返回截断内容。count 仅表示本次返回数量；name_contains 查询会在客户端跳过无匹配页，最多扫描 20 页、1000 个资源，找到含匹配项的一页即返回。scanned_count 是本次已扫描资源数，scan_limit_reached=true 表示达到扫描上限。has_more=true 表示还有未扫描资源，不保证后续存在匹配项；即使 count=0，也不能断言目标不存在或匹配已列全。需要继续时，用相同查询参数及返回的" +
		...
		ParamsOneOf: schema.NewParamsOneOfByParams(map[string]*schema.ParameterInfo{
			...
			"action":     {Type: schema.String, Required: true, Enum: []string{"list", "get"}, Desc: "list 获取一批列表，name_contains 可在客户端跨页查找名称片段；get 获取指定资源，默认返回摘要"},
			...
			"name_contains":  {Type: schema.String, Desc: "可选，仅 list；按名称字面包含匹配，大小写敏感，1–253 字节，允许字母、数字、点、下划线和连字符。例如 cwlicense。不传则保持普通分页列表；不能与 name 同用。"},
			"limit":          {Type: schema.Integer, Desc: "list 服务端每页条数，也是本次匹配结果上限，默认 50，范围 1–100；过滤时空匹配页由客户端继续扫描。get 不传"},
